...
VEILRendezvousTable::cp_rdv_entry(String s, ErrorHandler* errh){
	...
	veil_chatter("[Warning!]: Assumes that length of VID = 6 bytes, switch vid length = 4 bytes, size of int on the machine = 4 bytes\n");
	return 0;
}
	...
	VID *src, VID *dest)
{
	...
	if (oldEntry != NULL){
		...
		veil_chatter("[RendezvousTable][Edge Refresh] End1 VID: |%s| --> End2 VID: |%s| \n",src->vid_string().c_str(),dest->vid_string().c_str());
	}else{
		veil_chatter("[RendezvousTable][New Edge] End1 VID: |%s| --> End2 VID: |%s|\n",src->vid_string().c_str(),dest->vid_string().c_str());
	}
		...
		if(k == svid->logical_distance(&edge.dest) && 
		   svid->logical_distance(&edge.src) < k){
			...
			if ((temp1 ^ temp2) <= bestXORDist){
				...
				veil_chatter("[RendezvousTable][Gateway Selection] Found a better GW |%s| than |%s| for |%s| at level %d\n", edge.src.vid_string().c_str(),gateway->vid_string().c_str(),svid->vid_string().c_str(),k);
...
VEILRendezvousTable::expire(Timer *t, void *data) 	
{
	...
	veil_chatter("[RendezvousTable][Timer Expired] End1 VID: |%s| --> End2 VID: |%s| \n",edge->src.vid_string().c_str(),edge->dest.vid_string().c_str());
